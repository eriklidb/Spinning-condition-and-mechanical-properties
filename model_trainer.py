...
class ModelTrainer():
    ...
    def train_model(self, ds: Dataset, params: list[dict[str, Any]]) -> list[MultiOutputRegressor]:
        X, Y = ds()
        outer_models = []
        for outer_fold, (train_idx, test_idx) in enumerate(self._outer_cv.split(X)):
            X_train, X_test = X.iloc[train_idx], X.iloc[test_idx]
            ...
            params[outer_fold].update({'early_stopping':False,
                        'random_state':self._random_state,
                        'categorical_features':ds.categorical_columns})
            final_model = MultiOutputRegressor(HistGradientBoostingRegressor(**params[outer_fold]))
